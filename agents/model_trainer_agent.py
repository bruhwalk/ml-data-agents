...
Reads AL experiment results, uses Claude API to choose the best model,
trains it on all labeled data, and saves artifacts (model, metrics, charts, report).

Skills:
    - select_model(al_results, df) → recommendation (via Claude API)
...
class ModelTrainerAgent:
    ...
    Skills:
        - ``select_model(al_results, df)`` — LLM picks best model
    ...
    def select_model(
        ...
        """Use Claude API to choose the best model based on AL results and data.
  ...
  "reasoning": "<2-3 sentences>"
    ...
    def report(
        ...
        lines = [
            ...
            "## Model Selection (LLM)",
            "",
            f"Model chosen by Claude API: **{model_name}** (`{metrics['model']}`)",
            "",
