...
LLM-powered model selection: Claude API analyzes the task and data to choose
the best model and seed size for the experiment.

Skills:
    - select_model(df, task_type) → model name + seed size (via Claude API)
...
Usage::
    ...
    recommendation = agent.select_model(df, task_type="News topic classification")
...
class ActiveLearningAgent:
    ...
    Skills:
        - ``select_model(df, task_type)`` — LLM chooses model + seed size
    ...
    def select_model(
        ...
        """Use Claude API to analyze data and recommend model + seed size.
  ...
  "reasoning": "<2-3 sentences explaining your choice>"
    ...
    def _generate_report(
        ...
        lines.extend([
            "## Conclusion",
            "",
            f"Best strategy: **{best[0]}** (F1 = {best[1]['f1']:.4f})",
            "",
            "Active learning allows achieving target quality with fewer labeled examples "
            "by prioritizing the most informative data points for annotation.",
            "",
        ])
        ...
        if self._llm_recommendation:
            ...
            lines.extend([
                "## Model Selection (LLM)",
                "",
                f"Model chosen by Claude API: **{MODELS.get(rec['model'], {}).get('name', rec['model'])}** (`{rec['model']}`)",
