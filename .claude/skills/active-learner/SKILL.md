...
Claude API analyzes the dataset and task to choose the best model and seed size.
...
Claude API analyzes the dataset (class distribution, imbalance, text lengths, sample texts)
and chooses the best model and seed size:

```python
agent = ActiveLearningAgent()
recommendation = agent.select_model(df_labeled, task_type=task_type)
# → {"model": "logreg_balanced", "seed_size": 80, "reasoning": "..."}
```
...
- `nb` — Multinomial Naive Bayes (fast, works well with small datasets)
...
Execute:
```bash
python .claude/skills/eda-generator/scripts/run_notebook.py notebooks/al_experiment.ipynb
```
...
Active Learning experiment complete!
   LLM selected: Logistic Regression (balanced), seed=80
     Reason: "8 classes with 50:1 imbalance, balanced weights prevent minority class collapse"

   Dataset: 10,585 rows (seed=80, pool=8,388, test=2,117)
   Strategies: entropy, margin, random
   Iterations: 5 (batch_size=20)

   Final metrics (at 180 labeled):
     entropy: accuracy=0.89, F1=0.88
     margin:  accuracy=0.88, F1=0.87
     random:  accuracy=0.85, F1=0.83

   Savings:
     entropy reaches random's F1 at 120 examples (33% fewer)
     margin reaches random's F1 at 140 examples (22% fewer)
...
API key: `ANTHROPIC_API_KEY` from `.env` (for model selection)
