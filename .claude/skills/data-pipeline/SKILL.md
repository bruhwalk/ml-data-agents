...
Examples:
- "/data-pipeline собери датасет для классификации новостей"
- "запусти полный пайплайн для sentiment analysis"
- "run the full data pipeline for spam detection"
...
```
User request: "собери датасет для классификации новостей по темам"
         │
         ▼
┌─────────────────────────────────┐
...
Follow the **dataset-collector** skill (`.claude/skills/dataset-collector/SKILL.md`).
...
Follow the **label-master** skill (`.claude/skills/label-master/SKILL.md`).
...
Pipeline complete!
   ...
   Task: News topic text classification
   Sources: 3 (HuggingFace, Kaggle, API)
   Raw data: 10,585 rows
   Cleaned: 10,555 rows (30 removed)
   Labeled: 10,555 rows (7 classes, 14 disputed)
   AL: entropy saves 27% vs random
   Final model: accuracy=0.89, F1=0.77
   ...
   The pipeline is reproducible:
     python scripts/run_pipeline.py "News topic text classification"
```
