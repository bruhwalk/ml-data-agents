...
Создать `.env`:
```
ANTHROPIC_API_KEY=sk-...          # обязательно — разметка, выбор модели по данным и метрикам.
...
| Тип | Метод | Пример |
|-----|-------|--------|
| HuggingFace | `load_dataset(name, source="hf")`
| Kaggle | `load_dataset(name, source="kaggle")` 
| Web scraping | `scrape(url, selector)` 
| REST API | `fetch_api(endpoint, params)` 
| RSS/Atom | `fetch_rss(url)` 
...
| `source` | str | Идентификатор источника (например `huggingface:imdb`) |
...
**Авторазметка** (`auto_label`) — отправляет тексты в Claude API батчами. LLM возвращает метку + confidence (0.0–1.0). Добавляет 3 колонки:
