"""Full ML data pipeline: collect → clean → label → AL → train.

Usage::

    python run_data_pipeline.py "News topic text classification"
    python run_data_pipeline.py "Sentiment analysis of product reviews"

Equivalent to /data-pipeline in Claude Code, but runs standalone.
Human-in-the-loop checkpoints at steps 1-3, steps 4-5 are autonomous.
"""
...
def run_step3_labeling(task_description: str) -> pd.DataFrame:
    ...
    if metrics.low_confidence_count > 0:
        ...
        if ls_choice == "y":
            ...
            import subprocess
            try:
                ls_script = ROOT / ".claude/skills/label-master/scripts/start_ls.py"
                subprocess.run(
...
def run_step4_active_learning() -> None:
    ...
    with open(config_path) as f:
        config = yaml.safe_load(f)
    task_type = config.get("task", {}).get("type", "text classification")

    try:
        recommendation = agent.select_model(df, task_type=task_type)
        model_key = recommendation.get("model", "logreg")
        seed_size = recommendation.get("seed_size", 50)
        print(f"  LLM selected: model={model_key}, seed_size={seed_size}")
        print(f"  Reasoning: {recommendation.get('reasoning', '')[:200]}")
    except Exception as e:
        print(f"  LLM model selection failed ({e}), using defaults")
        model_key = "logreg"
        seed_size = 50

    # Run experiment
    agent = ActiveLearningAgent(model=model_key)
    results = agent.compare_strategies(df, seed_size=seed_size, n_iterations=5, batch_size=20)

    # Generate report
    artifacts = agent.report(results, output_dir=ROOT / "data/active_learning")
...
# ─────────────────────────────────────────────────────────────
# Step 5: Model Training (autonomous)
# ─────────────────────────────────────────────────────────────
def run_step5_model_training() -> None:
    """Train final model using agent class directly."""
    ...
    result = agent.run(
        parquet_path=ROOT / "data/labeled/labeled.parquet",
        task_type=task_type,
    )
    ...
    print(f"  F1 (weighted): {metrics.get('f1_weighted', '?')}")
...
def generate_pipeline_report(task_description: str) -> None:
    """Generate PIPELINE_REPORT.md in project root."""
    lines = [
    ...
    al_dir = ROOT / "data/active_learning"
    if (al_dir / "REPORT.md").exists():
        for hist_file in sorted(al_dir.glob("history_*.json")):
...
def main(task_description: str) -> None:
    ...
    print("Saved task type to config.yaml")
    ...
    print()
    print("  Full report: PIPELINE_REPORT.md")
    print()
    print("  Reproducible with:")
    print(f'    python run_data_pipeline.py "{task_description}"')


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print('Usage: python run_data_pipeline.py "Task description"')
        print('Example: python run_data_pipeline.py "News topic text classification"')
        sys.exit(1)
    main(sys.argv[1])
